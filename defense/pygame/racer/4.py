#4. Добавить новый тип препятствий – ямы
import pygame
import random
import time
...
class Hole(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image.fill((50, 50, 50))  # Серый цвет
        self.rect = self.image.get_rect()
        self.rect.left = random.randint(0, WIDTH - 50)
        self.rect.bottom = 0
        self.speed = 4
...
for _ in range(2):  # 2 монеты на экране одновременно
    coin = Coin()
...
while running:
    ...
    if player.rect.colliderect(hole.rect):
        player.lives -= 1
        hole.rect.left = random.randint(0, WIDTH - 50)
        hole.rect.bottom = 0
