#1. Добавить бонус-монеты, дающие временное ускорение
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.value = random.choice([1, 2, 5, "boost"])  # Теперь есть "ускорение"
        self.generate_random_rect()
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            player.speed = 5  # Возвращаем обычную скорость
    ...
    for coin in coin_sprites:
        if player.rect.colliderect(coin.rect):
            if coin.value == "boost":
                player.speed += 3  # Временное ускорение
                pygame.time.set_timer(pygame.USEREVENT, 3000)  # Через 3 секунды сброс
        else:
                player.coins_collected += coin.value  # Обычная монета

        coin.generate_random_rect()  # Убираем собранную монету
