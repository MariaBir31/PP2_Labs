#7. Уровни добавляют новые стены (динамический лабиринт)
        ...
        if new_head in snake or new_head[0] < 0 or new_head[0] >= WIDTH or new_head[1] < 0 or new_head[1] >= HEIGHT:
            ...
            if new_head == food:
                ...
                if score // 30 + 1 > level:
                    level += 1
                    speed += 2
                    new_wall = (random.randint(0, WIDTH // BLOCK_SIZE - 1) * BLOCK_SIZE,
                random.randint(0, HEIGHT // BLOCK_SIZE - 1) * BLOCK_SIZE)
                if new_wall not in snake and new_wall != food:  # Не появляться в змейке и на еде
